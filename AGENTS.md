...
guessed. A guessed allow-list is the thing that breaks CI for an unrelated reason, and this repository has a
specific reason to care: the prune phase reads a failed run as "this path was needed", so a
build that fails because a host was missing from an allow-list produces a weaker sandbox.
