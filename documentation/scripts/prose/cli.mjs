 ...
 */
const SOURCE_FILES = [
    'docusaurus.config.ts', 'sidebar-user.ts', 'sidebar-contributor.ts',
];
 ...
 * can still fail on a count. The ceiling is what stops the findings this documentation carries
 * from quietly growing, and it is checked in BOTH directions, as the comment in `lint` below
 * says: a count above its ceiling fails, and so does a count below one, because a ceiling left
 * at the old figure leaves room the next change can spend. Either way the number is re-recorded
