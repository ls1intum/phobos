...
import { SITE, scanCategory, scanPage, scanSource } from './scan.mjs';
...
/**
 * The writing-rules document and the rules it describes, held to each other.
 *
 * The document is the standard and `rules.mjs` is the implementation of the decidable part of it.
 ...
 */
describe('the document and the rules agree', () => {
    // WRITING-RULES.md sits beside this site's own README rather than under docs/. The
    // sidebars of both guides are fixed by agreement, and a published page would add a section
    // to one of them; these rules are documentation for whoever edits this site, which is what
    // that README is for. Everything below reads the file the same way either location would.
    const PAGE = path.join(SITE, 'WRITING-RULES.md');
...
describe('terms of art keep their spelling', () => {
    ...
    test('a student submission names a use case, not a second name for the concept', async () => {
        const rules = await rulesFor('Grading a student submission is one use case.\n');
        assert.deepEqual(rules, []);
    });

    test('supervised command is a second name for the sandboxed command', async () => {
        const rules = await rulesFor('The layer watches the supervised command.\n');
        assert.deepEqual(rules, ['canonical-terms']);
    });

    test('graded command is a second name for the sandboxed command', async () => {
        const rules = await rulesFor('The directory lies outside the graded command reach.\n');
        assert.deepEqual(rules, ['canonical-terms']);
    });
});
...
describe('sentence openers', () => {
    test('a sentence opening with Because is reported', async () => {
        const rules = await rulesFor('Because the policy names it, Landlock permits the open.\n');
