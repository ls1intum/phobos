 ...
 *
 * The rules themselves are the AET general writing rules, taken from the Ares 2
 * documentation so that the two sister sites read alike. They were written for exercise text
 * that students read, and two of them are retargeted here: the canonical term is Phobos's
 * "sandboxed command" rather than Ares's "student code", and the abbreviation list is the one
 ...
 * pull request template. Where `artifact` names a GitHub Actions artefact upload it sits in a
 * workflow file, which is never scanned.
 */
...
export const WORD_RULES = [
    ...
    {
        id: 'canonical-terms',
        // Phobos sandboxes any executable program, and the documentation says so. The words
        // that pull it back towards one use case are therefore the ones held to one name: the
        // thing inside the sandbox is "the sandboxed command", whether it is a student's
        // submission, a build step or a data job. A page that discusses grading still names a
        // student submission, because that is the use case rather than a second name for the
        // concept, and the source's own message text is quoted inside code, which is never
        // scanned.
        level: 'enforced',
        pattern: /\b(?:graded|restricted|supervised|confined)\s+(?:code|command|program)\b/gi,
        message: (hit) => `"${hit}" is the same concept as "sandboxed command". Write `
            + '"sandboxed command", so no page reads as though Phobos only ran one kind of '
            + 'program.',
        allow: [],
    },
    {
    ...
    },
    {
        ...
        message: (hit) => `"${hit}" addresses the reader as a group. Write "you", meaning `
            + 'whoever this guide is for. Where it means the Phobos project itself, leave it.',
        allow: [
...
export const ABBREVIATIONS = [
    { short: 'ABI', expansion: /application binary interface/i, write: 'application binary interface (ABI)' },
    ...
    { short: 'CIDR', expansion: /classless inter[- ]domain routing/i, write: 'Classless Inter-Domain Routing (CIDR)' },
    ...
    { short: 'IPC', expansion: /inter[- ]process communication/i, write: 'inter-process communication (IPC)' },
    ...
    { short: 'SNI', expansion: /server name indication/i, write: 'Server Name Indication (SNI)' },
